...
public class SwingArena extends JPanel
{
    ...
    public static BlockingQueue<Click> clickListBlockingQueue = new LinkedBlockingDeque<>(5);
    private static final String ROBOTIMG = "1554047213.png";
    private static final String FORTRESSIMG = "rg1024-isometric-tower.png";
    private ImageIcon robot1, fortress;
    ...
    KillRobot killRobot = new KillRobot();
    ...
    public SwingArena()
    {
        ...
        // 'resources/' directory.
        URL url = getClass().getClassLoader().getResource(ROBOTIMG);
        URL url2 = getClass().getClassLoader().getResource(FORTRESSIMG);
        if(url == null)
        {
            throw new AssertionError("Cannot find image file " + ROBOTIMG);
        }
        else if (url2 == null)
        {
            throw new AssertionError("Cannot find image file " + FORTRESSIMG);
        }
        fortress = new ImageIcon(url2);
                ...
                public void mouseClicked(MouseEvent event)
                {
                    ...
                    if(gridX < gridWidth && gridY < gridHeight)
                    {
                        for(ArenaListener listener : listeners)
                        {
                            ...
                            newClick = new Click(gridX, gridY);

                            try {
                                //---------- Creating a new fire and adding it into the queue----//
                                clickListBlockingQueue.put(newClick);
                                executor.execute(killRobot);
        ...
        if(!RobotToQueue.robotListBlockingQueue.isEmpty()) {
            Iterator<Robot> robotObj = RobotToQueue.robotListBlockingQueue.iterator();
                ...
                while (robotObj.hasNext()) {
                    ...
                    drawImage(gfx, robot1, robo.getX(), robo.getY());
                    String uid = Integer.toString(robo.getUniqueid());
                    drawLabel(gfx, uid, robo.getX(), robo.getY());
                    drawLine(gfx,4, 4, newClick.getX(), newClick.getY());
                }
        }
    ...
    private void drawLabel(Graphics2D gfx, String label, double gridX, double gridY)
    {
        ...
        gfx.drawString(label, 
            ...
            (int) ((gridY + 1) * gridSquareSize) + fm.getHeight());
    }
